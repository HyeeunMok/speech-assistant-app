...
def record_audio(ask=False):
    with sr.Microphone() as source:
        ...
        try:
            ...
            assistant_speak('Sorry, my speech assistant service is down.')
        return voice_data


def respond(voice_data):
    if 'what is your name' in voice_data:
        assistant_speak('My name is Hyeeun')
    ...
    if 'find location' in voice_data:
        ...
        webbrowser.get().open(url)
        assistant_speak('Here is the locaton of ' + location)
    if 'exit' in voice_data:
        assistant_speak('Good bye')
        exit()
